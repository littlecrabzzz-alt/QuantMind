...
迁移必须分阶段验收：先建立 Mac 只读供数和云端研究子集校验；再迁移完整归档与采集检查点、切换唯一采集写入者；最后逐项核验本地持有后释放云端非研究文件。未完成迁移的文件不可删除。当前云端采集尚在过渡期，不能把新供数入口上线称为全量主库已经迁移。详细状态见 [Mac 数据源说明](docs/tushare-mac-source.md)。
