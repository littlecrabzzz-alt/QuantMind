...
Tushare 全量原始数据与历史归档改由 Mac 本地持有，之后迁至 NAS；云服务器只保留研究需要的数据和受限缓存，并从本地归档取数。此决定取代下文关于 Tushare 永久以云端全量库为主的旧约定，不改变 PostgreSQL 业务库、QuantDB 当前更新链路和研究结果的归属。禁止将 Mac 沙盒业务库回灌云端。
...
| QuantDB 基础数据 | 复用快照校验，每15分钟及唤醒后从云端发布/拉取限定 Parquet 版本；本地空闲时应用并补齐行情表和 Qlib | 以 `.local-dev/QUANTDB_SYNC.json` 的 `applied` 和实际数据日期验收；本地修改冲突、活动任务或失败时保留数据并报错，下轮重试 |
...
| 接入或补齐行情、财务等数据 | 先在隔离输入上验证采集/转换代码；QuantDB 走云端既有入口，Tushare 走已验证 Mac 归档采集入口 | 原始对象、检查点与缺口留在各自数据所有者；校验成功才发布可读版本，禁止跨节点覆盖运行中的库 |
...
本地/云端都可通过各自的研究工作台创建研究；输入与结果落在各自节点。QuantDB 行情采集由云端维护，Tushare 全量采集由 Mac 归档所有者维护。正式研究记录应包含输入快照或 `release_id`、数据截止时间、代码版本、参数与任务 ID。Qlib 是派生缓存，复用 `QlibDataBuilder.build_all` 的受限构建、完整校验和原子发布，不手工先替换日历。实盘开关保持关闭。

## 数据如何更新，什么时候会落后

- **QuantDB 日常更新**：云端 A 股配置每天北京时间 03:00 采集，错过时间可当日补派；市场同步走现有 worker 的独立队列，失败任务不会无限重投。Mac `com.quantmind.snapshot-pull` 每15分钟按日历触发，休眠错过的触发在唤醒后合并补跑；只发布/拉取限定 QuantDB Parquet，再在沙盒空闲时应用；复用文件哈希、增量传输和既有 PG/Qlib 入口，不依赖完整快照成功，也不停止云端整套服务。手动同一路径为 `python3 scripts/quantdb_refresh.py refresh`。
- **应用与保护**：本地已有行情修改发生冲突则拒绝覆盖；旧 QuantDB 留在 `.local-dev/quantdb-before-*`，只向本地行情表补新增日期，不恢复或覆盖业务库。任务运行或后端未启动时保留下载、延后应用。若已应用行情但派生计算失败，记录待恢复状态并暂停本地API/相关worker，下轮允许在停机状态继续计算，全部成功才恢复原服务，避免新旧数据混用。研究固定输入目录保持原样；查看 `.local-dev/QUANTDB_SYNC.json`、应用真实最新日与代表标的数据，不能用下载成功替代验收。旧版保留，空间不足时先报告，不自动删除本地成果。
...
- **Tushare**：Mac 的 `com.quantmind.tushare-archive` 在 `~/Library/Application Support/QuantMind/tushare` 持续采集并发布固定版本；以后迁 NAS 时仍须校验唯一写入者。云端 `tushare-research-cache.timer` 每轮结束约 15 分钟后，从 Mac 私有供数入口获取受限研究子集。Mac 休眠或离线时云端保留已校验缓存，不回退到供应商采集；不能承诺离线时自动追新。两端 `CURRENT.json` 更新不改变正在进行的研究固定版本。存量迁移成功不代表全部历史、权限、字段或时间点数据齐全；部署与验收见 [Mac 数据源说明](docs/tushare-mac-source.md)。
