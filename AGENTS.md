...
## Tushare 存储方向变更（2026-09-16 用户确认）

Tushare 全量原始数据与历史归档改由 Mac 本地持有，之后迁至 NAS；云服务器只保留研究需要的数据和受限缓存，并从本地归档取数。此决定取代下文关于 Tushare 永久以云端全量库为主的旧约定，不改变 PostgreSQL 业务库和研究结果的归属；QuantDB 更新方向由下述 2026-09-27 本地采集规则接替。禁止将 Mac 沙盒业务库回灌云端。

迁移必须分阶段验收：先建立 Mac 只读供数和云端研究子集校验；再迁移完整归档与采集检查点、切换唯一采集写入者；最后逐项核验本地持有后释放云端非研究文件。未完成迁移的文件不可删除。2026-09-17 已完成存量逐文件校验及唯一写入者交接：Mac 的 ARCHIVE_AUTHORITY.json 已验证，云端 ARCHIVE_RELOCATED.json 持久停写；两端应用读取已切换。云端已校验的旧原始文件及 11 个旧数据库/检查点副本已释放；仍保留的完整备份等待正常轮转。本地历史补采继续，迁移完成不代表全部历史已采齐。详细状态见 [Mac 数据源说明](docs/tushare-mac-source.md) 和 `coordination/tushare-data/`。

## 市场数据本地采集（2026-09-27 用户确认）

QuantDB、Tushare 和 BTC/ETH 日线均由 Mac 唯一采集。云端不得直接访问供应商补采；只接收已校验数据版本，复用现有 PG、Qlib、H5 和市场分析入口。Mac 离线时云端保留旧版并显示截止日，不能自行恢复上游调度。本地业务库仍不回灌云端。

Mac `com.quantmind.market-source` 管理 QuantDB 03:00、BTC/ETH 08:15（北京时间）的日更；每15分钟与唤醒后检查，失败阶段重试。旧 `com.quantmind.snapshot-pull` 已退出日常行情更新，完整业务快照继续按需恢复使用。Tushare 仅自动采集配置白名单的18个研究接口，暂停公告附件、文本和其他广泛补采，保留历史与检查点。操作与恢复见 [本地数据源运行说明](docs/local-market-source.md)。
...
| QuantDB 基础数据 | Mac 独立源目录采集，按固定清单校验并发送云端；两端分别补齐行情表、Qlib 和市场分析 | 以 `.local-dev/QUANTDB_SYNC.json` 的 `applied` 和实际数据日期验收；本地修改冲突、活动任务或失败时保留数据并报错，下轮重试 |
...
| Tushare 完整归档 | Mac 原生采集并发布固定版本，云端经私有 SSH 通道取得研究子集 | 完整历史、原始对象、附件与检查点在 Mac；两端按 `release_id` 读取，不自动导入 PostgreSQL/沙盒，不恢复旧云端采集或镜像任务 |
...
后续按“两端独立开发和计算 → 有价值的数据按项目汇总云端 → 两端取得共享版本”的规范实施。临时测试数据留在本节点，云端开发也要隔离；指定共享项目的方案版本和已完成成果将支持联网增量同步，正式基础数据由 Mac 采集后按固定版本送达两端，云端业务数据仍由云端维护。
...
| 接入或补齐行情、财务等数据 | 先在隔离输入上验证采集/转换代码；QuantDB 与 BTC/ETH 走 Mac 隔离采集容器，Tushare 走已验证 Mac 原生归档入口 | 原始对象、检查点与缺口留在各自数据所有者；校验成功才发布可读版本，禁止跨节点覆盖运行中的库 |
...
本地/云端都可通过各自的研究工作台创建研究；输入与结果落在各自节点。市场数据采集统一由 Mac 维护，云端仅消费已校验版本。正式研究记录应包含输入快照或 `release_id`、数据截止时间、代码版本、参数与任务 ID。Qlib 是派生缓存，复用 `QlibDataBuilder.build_all` 的受限构建、完整校验和原子发布，不手工先替换日历。实盘开关保持关闭。

## 数据如何更新，什么时候会落后

- **QuantDB 日常更新**：Mac 每天北京时间03:00采集现有23个数据集，`com.quantmind.market-source` 每15分钟和唤醒后补查。固定版本经SHA-256校验送达云端，两端分别派生PG/Qlib/市场分析。手动补查用 `python3 scripts/local_market_source.py tick`；不要重新安装旧云端采集或 snapshot-pull。
- **应用与保护**：本地已有行情修改发生冲突则拒绝覆盖；旧 QuantDB 留在 `.local-dev/quantdb-before-*`，只向本地行情表补新增日期，不恢复或覆盖业务库。任务运行或后端未启动时保留下载、延后应用。若已应用行情但派生计算失败，记录待恢复状态并暂停本地API/相关worker，下轮允许在停机状态继续计算，全部成功才恢复原服务，避免新旧数据混用。研究固定输入目录保持原样；查看 `.local-dev/QUANTDB_SYNC.json`、应用真实最新日与代表标的数据，不能用下载成功替代验收。新链路登记的中转与回滚副本保留最近两版；迁移前旧备份和研究固定输入不自动删除。
...
- **完整数据库快照**：仍是独立恢复基线，包含业务数据库等；整项目快照容量不足时可以暂停，但不能连带关闭上述 QuantDB 日常更新。完整快照手动按需下载，不再串行占用日常行情定时器。下载区仍为 `~/Library/Application Support/QuantMind/cloud-snapshots`，其中 `quantdb/` 保存限定基础数据版本。
- **忙时不会强行做完整快照**：存在活动任务时退出 75、保留旧版，当前计划等下一次 07:00 再试，没有保证当天一定出新快照的补跑机制。Mac 拉取成功可能只是确认“仍是旧的已校验版本”；检查快照 ID 与日期，不能只看任务已安装或退出码为 0。
- **Tushare**：Mac 的 `com.quantmind.tushare-archive` 在 `~/Library/Application Support/QuantMind/tushare` 持续采集并发布固定版本；以后迁 NAS 时仍须校验唯一写入者。云端 `tushare-research-cache.timer` 每轮结束约15分钟后，从 Mac 私有供数入口获取受限研究子集；本地研究版每小时发布，全归档每天发布。Mac 休眠或离线时云端保留已校验缓存，不回退到供应商采集；不能承诺离线时自动追新。两端 `CURRENT.json` 更新不改变正在进行的研究固定版本。存量迁移成功不代表全部历史、权限、字段或时间点数据齐全；部署与验收见 [Mac 数据源说明](docs/tushare-mac-source.md)。
...
- **手动刷新**：只拉已发布完整快照用 `python3 scripts/dual_node_snapshot.py pull`；`bash scripts/dual-node.sh snapshot-refresh` 还会在云端创建完整快照，需要协调停写窗口。Mac 离线/休眠时无法拉取，云端业务作业继续使用旧版；恢复后本地补采和推送。定时客户端是独立安装副本，修改客户端代码后须按对应安装说明更新，Git 同步不会自动升级它。
...
- **市场数据采集所有权**：正式云端角色 `authority` 禁止供应商采集与定时派发；QuantDB/BTC-ETH 的每日触发由 Mac `com.quantmind.market-source` 持有。Redis 中旧市场调度关闭，不能通过页面重新启用。独立沙盒手动采集须指定独立输出。
