...
迁移必须分阶段验收：先建立 Mac 只读供数和云端研究子集校验；再迁移完整归档与采集检查点、切换唯一采集写入者；最后逐项核验本地持有后释放云端非研究文件。未完成迁移的文件不可删除。2026-09-17 已完成存量逐文件校验及唯一写入者交接：Mac 的 ARCHIVE_AUTHORITY.json 已验证，云端 ARCHIVE_RELOCATED.json 持久停写；两端应用读取已切换。本地历史补采和云端旧副本清理仍在继续，迁移完成不代表全部历史已采齐。详细状态见 [Mac 数据源说明](docs/tushare-mac-source.md) 和 `coordination/tushare-data/`。
