...
- 安全窗口：先停 Beat、取消 `tushare_acquire` consumer，等待 active/reserved 连续两轮为空后正常停止 worker；排空过程没有调用 revoke、强杀、重启或 OOM。恢复时 Redis 中一个已超过 110 秒有效期的旧排队消息被 Celery 标记为 `revoked/expired` 后丢弃，未运行、未调用上游也未写入部分结果。三份 360 项清单均先通过 plan-only，固定任务、配置、helper、prepare 与 release 哈希。
