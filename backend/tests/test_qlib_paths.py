...
def test_missing_fixed_snapshot_does_not_switch_to_ready_live_cache(tmp_path, monkeypatch):
    monkeypatch.delenv("QLIB_PROVIDER_URI", raising=False)
    monkeypatch.setattr(qlib_paths, "_PROJECT_ROOT", tmp_path)
    _make_ready_provider(tmp_path / "data/qlib/cn_data")
    frozen = str(tmp_path / "research/frozen-input")
    assert qlib_paths.fallback_to_ready_provider_uri(frozen) == frozen
