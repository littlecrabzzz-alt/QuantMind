...
def test_classification_keeps_binary_labels_and_uses_classification_objectives() -> None:
    source = TRAIN_SCRIPT.read_text(encoding="utf-8") + (ROOT / "docker/training/data/loading.py").read_text() + (ROOT / "docker/training/model_trainers/trainers_gbdt.py").read_text()
...
def test_labels_and_split_gap_share_t_plus_one_execution_convention() -> None:
    ...
    assert "_purge_label_tail(train_df, cfg)" in splits_source
    assert "_purge_label_tail(val_df, cfg)" in splits_source
