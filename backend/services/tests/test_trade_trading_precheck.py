...
@pytest.mark.asyncio
async def test_trading_precheck_simulation_checks_database_model_pool_quotes_and_signals(monkeypatch, tmp_path):
    model_dir = tmp_path / "model_qlib"
    model_dir.mkdir(parents=True)
    (model_dir / "model.lgb").write_bytes(b"fake_model")
    monkeypatch.setenv("MODELS_PRODUCTION", str(model_dir))
    ...
    fake_db = _FakeDb([{"ok": 1}, {"sim_orders": True, "sim_trades": True, "simulation_fund_snapshots": True}])
...
@pytest.mark.asyncio
async def test_preflight_simulation_includes_database_check(monkeypatch):
    ...
    assert any(item["key"] == "db" and item["ok"] for item in result["checks"])
