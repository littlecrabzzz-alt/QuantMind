...
class TestAPIExtendedIntegration:
    ...
    def test_admin_dashboard_integration(self, client):
        ...
        async def healthy_infra(service):
            return {"name": service if isinstance(service, str) else service.get("name", "infra"),
                    "status": "healthy", "score": 100}

        with patch("httpx.AsyncClient.get", _mock_get), patch(
            "backend.services.api.routers.admin.dashboard._fetch_tcp_service_health", healthy_infra
        ), patch(
            "backend.services.api.routers.admin.dashboard._fetch_celery_health", healthy_infra
        ):
