...
def fallback_to_ready_provider_uri(
    ...
    if is_qlib_provider_ready(candidate):
        return candidate
    # Explicit research snapshots must never silently switch to live data.
    raw = str(provider_uri or "").rstrip("/")
    subdir = {"CN": "cn_data", "HK": "hk_data", "US": "us_data", "CRYPTO": "bc_data", "FUTURES": "futures_data"}.get(market, "cn_data")
    defaults = {"", f"/data/qlib/{subdir}", str(_PROJECT_ROOT / "data" / "qlib" / subdir), *_LEGACY_CN_PREFIXES}
    if raw not in defaults or os.getenv("QLIB_PROVIDER_URI", "").strip():
        return candidate
