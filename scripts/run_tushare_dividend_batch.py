#!/usr/bin/env python3
"""Plan or execute one hash-pinned dividend history batch."""
...
from scripts import prepare_tushare_dividend_batch as preparation  # noqa: E402
...
def _execute(
    ...
    receipt = {
        ...
        "market_counts": verified["market_counts"],
        ...
        if not execute:
            for target in (
                ...
                guards.enter_context(
                    patch(
                        target,
                        side_effect=AssertionError("Offline dividend batch plan"),
                    )
                )
        verified = preparation.verify_manifest(manifest, manifest_sha256)
