#!/usr/bin/env python3
"""Plan or execute one hash-pinned IRM Q&A batch."""
...
from scripts import prepare_tushare_irm_qa_batch as preparation  # noqa: E402
...
def _verify_authority_jobs(pipeline, records):
    for expected in records:
        ...
        actual = {
            ...
            "partition_child": bool(saved["partition_child"]),
...
def _execute(
    ...
    with os.fdopen(descriptor, "a+") as lock:
        ...
        if preparation.rate_contracts(json.loads(config_bytes)) != verified[
            ...
            raise ValueError("IRM Q&A rate contracts changed")
    ...
    receipt = {
        ...
        "axis_counts": verified["axis_counts"],
        "partition_counts": verified["partition_counts"],
        "logical_request_counts": verified["logical_request_counts"],
        "api_rate_contracts": verified["api_rate_contracts"],
        ...
        with ExitStack() as guards:
            for target in (
                ...
                guards.enter_context(
                    patch(
                        target,
                        side_effect=AssertionError("Offline IRM Q&A batch plan"),
                    )
                )
            verified = preparation.verify_manifest(manifest, manifest_sha256)
