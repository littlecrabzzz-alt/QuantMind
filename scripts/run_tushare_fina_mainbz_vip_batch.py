#!/usr/bin/env python3
"""Plan or execute one hash-pinned fina_mainbz_vip quarterly batch."""
...
from scripts import prepare_tushare_fina_mainbz_vip_batch as preparation  # noqa: E402
...
def _verify_release(root, verified):
    ...
    actual = preparation.release_evidence(
        root, source["release_id"], source["release_manifest_sha256"]
    )
    if actual != {
        "release_id": source["release_id"],
        "release_manifest_sha256": source["release_manifest_sha256"],
        "current_pointer_sha256": source["current_pointer_sha256"],
    }:
        raise ValueError("Pinned fixed-release evidence changed")
...
def _verify_source_inventory(pipeline, source):
    records = preparation.eligible_records(pipeline.db)
    task_ids = sorted(record["task_id"] for record in records)
    if (
        len(records) != source["eligible_tasks"]
        or digest(json_bytes(task_ids)) != source["eligible_task_ids_sha256"]
    ):
        raise ValueError("Authority eligible task inventory changed")
...
def _execute(
    ...
    with os.fdopen(descriptor, "a+") as lock:
        ...
        if (
            ...
            raise ValueError("fina_mainbz_vip rate contracts changed")
        pipeline = pipeline_module.Pipeline(
            root, json.loads((REPO / "config/tushare-catalog.json").read_bytes())
        )
        try:
            _verify_source_inventory(pipeline, verified["source"])
    ...
    receipt = {
        ...
        "current_pointer_sha256": verified["source"]["current_pointer_sha256"],
        "api_counts": verified["api_counts"],
        "selected": verified["selected"],
        "boundaries": verified["boundaries"],
        "rate_contracts": verified["rate_contracts"],
...
def run_batch(
    ...
    *,
    ...
    if not all(
        ...
        raise ValueError(
            "Execute requires pinned release, manifest, task, config, helper "
            "and preparation values"
        )
    verified = preparation.verify_manifest(manifest, manifest_sha256)
    if (
