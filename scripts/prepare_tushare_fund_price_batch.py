...
ALLOWED_APIS = ("fund_daily", "fund_adj")
GROUP = "rrg"
EXCHANGE = "SSE"
DAYS_PER_API = 180
...
def _job_api_day(record):
    ...
    valid_params = (
        ...
        and set(params) == {"trade_date"}
    ) or (
        api == "fund_adj"
        and isinstance(params, dict)
        and set(params) == {"trade_date", "offset", "limit"}
        and type(params.get("offset")) is int
        and params["offset"] >= 0
        and params.get("limit") == 1000
        and params["offset"] % params["limit"] == 0
    )
    if (
        ...
        raise ValueError("Batch contains a non-fund-price task")
    return api, params["trade_date"]


def _is_initial(record):
    return record["job"]["api_name"] == "fund_daily" or record["job"]["params"][
        "offset"
    ] == 0
...
def verify_manifest(path, manifest_sha256):
    ...
    if (
        ...
        or manifest["kind"] != "fund_price_exact_batch"
    ...
    if (
        ...
        raise ValueError("Batch is not balanced across the two APIs")
...
def prepare(root, output, release_id, release_manifest_sha256, days_per_api=DAYS_PER_API):
    ...
    for row in rows:
        ...
        params = job.get("params") if isinstance(job, dict) else None
        api = job.get("api_name") if isinstance(job, dict) else None
        day = params.get("trade_date") if isinstance(params, dict) else None
        if api not in ALLOWED_APIS or not re.fullmatch(r"[0-9]{8}", str(day or "")):
            # Range/code diagnostics are valid queue work but not exact-day candidates.
            continue
        try:
            api, day = _job_api_day(record)
        except ValueError:
            # A code-split or otherwise different exact-day task means this date
            # already has partial work and must not enter the broad first-page batch.
            tainted.add((api, day))
            continue
        key = (api, day)
        if (
