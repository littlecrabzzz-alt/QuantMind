#!/usr/bin/env python3
"""Freeze pristine fina_mainbz_vip quarter/type tasks against one fixed release."""
...
MAX_BATCH_JOBS = MAX_BATCH_PERIODS * len(TYPES)
...
BOUNDARIES = {
    ...
    "request_exact_quarter_preserved": True,
...
def rate_contracts(config):
    ...
    if (
        ...
        raise ValueError("Unexpected fina_mainbz_vip rate contract")
...
def _validate_record(record):
    ...
    if (
        ...
        raise ValueError("Batch contains a non-pristine fina_mainbz_vip history task")
...
def _selected(records):
    ...
    periods = sorted({item["period"] for item in params})
    return {
        "jobs": len(records),
        "quarter_count": len(periods),
        "quarter_ids_sha256": digest(json_bytes(periods)),
        "type_counts": dict(sorted(Counter(item["type"] for item in params).items())),
        "report_period_min": min(periods),
        "report_period_max": max(periods),
        "complete_pdi_per_quarter": all(
            {item["type"] for item in params if item["period"] == period} == set(TYPES)
...
def verify_manifest(path, manifest_sha256):
    ...
    if (
        ...
        or manifest.get("kind") != "fina_mainbz_vip_history_exact_batch"
        ...
        != EXPECTED_API_RPM
        or manifest.get("rate_contracts", {}).get("api", {}).get("minimum_points")
        != 5000
        or not str(
...
def _fair_selection(inventory, periods):
    buckets = {}
    for record in inventory:
        params = _validate_record(record)
        bucket = buckets.setdefault(params["period"], {})
        if params["type"] in bucket:
            raise ValueError("Duplicate fina_mainbz_vip quarter/type identity")
        bucket[params["type"]] = record
    records = []
    for period in sorted(buckets, reverse=True):
        if set(buckets[period]) != set(TYPES):
            continue
        for kind in TYPES:
            records.append(buckets[period][kind])
        if len(records) == periods * len(TYPES):
            break
    return records
...
def prepare(
    root, output, release_id, release_manifest_sha256, periods=MAX_BATCH_PERIODS
):
    if type(periods) is not int or not 1 <= periods <= MAX_BATCH_PERIODS:
    ...
    with _read_lock(root):
        ...
        try:
            ...
            db.close()
    records = _fair_selection(inventory, periods)
    if len(records) != periods * len(TYPES):
        raise ValueError("Insufficient complete pristine fina_mainbz_vip quarters")
    eligible_ids = sorted(record["task_id"] for record in inventory)
    task_ids = sorted(record["task_id"] for record in records)
    manifest = {
        "schema_version": 1,
        "kind": "fina_mainbz_vip_history_exact_batch",
        "source": {
            **release,
            "api_name": API,
            "group_name": GROUP,
            ...
            "tries": 0,
            "attempts": 0,
            "selection": "quarter_desc_complete_pdi_pristine_tasks",
...
def main():
    ...
    parser.add_argument("--periods", type=int, default=MAX_BATCH_PERIODS)
