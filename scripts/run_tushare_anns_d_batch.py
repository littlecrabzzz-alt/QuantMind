#!/usr/bin/env python3
"""Plan or execute one hash-pinned anns_d split-child batch."""
...
from scripts import prepare_tushare_anns_d_batch as preparation  # noqa: E402
...
def helper_sha256():
    return exact_runner.helper_sha256(__file__)
...
def _execute(
    ...
    with os.fdopen(descriptor, "a+") as lock:
        ...
        if preparation.rate_contracts(json.loads(config_bytes)) != verified[
            ...
            raise ValueError("Announcement rate contracts changed")
        ...
        with ExitStack() as guards:
            for target in (
                ...
                guards.enter_context(
                    patch(
                        target,
                        side_effect=AssertionError("Offline anns_d batch plan"),
                    )
                )
            verified = preparation.verify_manifest(manifest, manifest_sha256)
