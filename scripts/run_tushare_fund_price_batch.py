#!/usr/bin/env python3
"""Plan or execute one hash-pinned fund price batch."""
...
from scripts import prepare_tushare_fund_price_batch as preparation  # noqa: E402
...
def _verify_release(root, verified):
    ...
    actual = preparation.calendar_evidence(
        root, source["release_id"], source["release_manifest_sha256"]
    )
    if actual != verified["calendar"]:
        raise ValueError("Pinned fixed-release calendar evidence changed")
...
def _execute(
    ...
    receipt = {
        ...
        "calendar_open_dates_sha256": verified["calendar"]["open_dates_sha256"],
        ...
        with ExitStack() as guards:
            for target in (
                ...
                guards.enter_context(
                    patch(
                        target,
                        side_effect=AssertionError("Offline fund-price batch plan"),
                    )
                )
            verified = preparation.verify_manifest(manifest, manifest_sha256)
