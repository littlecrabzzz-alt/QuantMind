#!/usr/bin/env python3
"""Plan or execute the hash-pinned six-leaf NPR history batch."""
...
from scripts import prepare_tushare_npr_history_leaf_batch as preparation  # noqa: E402
...
def _verify_authority_jobs(pipeline, records):
    ...
    if attempted:
        raise ValueError("NPR logical request gained an attempt")
...
def _execute(
    ...
    with os.fdopen(descriptor, "a+") as lock:
        ...
        if preparation.rate_contracts(json.loads(config_bytes)) != verified[
            ...
            raise ValueError("NPR rate contracts changed")
                ...
                guards.enter_context(
                    patch(
                        ...
                        side_effect=AssertionError(
                            "Offline npr history leaf batch plan"
                        ),
                    )
                )
            verified = preparation.verify_manifest(manifest, manifest_sha256)
