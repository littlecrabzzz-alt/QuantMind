#!/usr/bin/env python3
"""Freeze untouched announcement split children against one fixed release."""
...
def rate_contracts(config):
    ...
    if (
        ...
        raise ValueError("Unexpected anns_d rate contract")
...
def _validate_record(record):
    ...
    if (
        ...
        raise ValueError("Batch contains a non-announcement or non-pristine child")
...
def verify_manifest(path, manifest_sha256):
    ...
    if (
        ...
        or manifest.get("kind") != "anns_d_split_child_exact_batch"
...
def prepare(root, output, release_id, release_manifest_sha256, jobs=MAX_BATCH_JOBS):
    ...
    for record in eligible:
        ...
        if len(records) == jobs:
            break
    if not records:
        raise ValueError("No untouched announcement split children are pending")
    task_ids = sorted(record["task_id"] for record in records)
    logical_keys = [record["logical_key"] for record in records]
    manifest = {
        "schema_version": 1,
        "kind": "anns_d_split_child_exact_batch",
        "source": {
            **release,
            ...
            "selection": "deterministic_distinct_unattempted_split_children",
