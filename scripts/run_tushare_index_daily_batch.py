#!/usr/bin/env python3
"""Plan or execute one hash-pinned index_daily history batch."""
...
from scripts import prepare_tushare_index_daily_batch as preparation  # noqa: E402
...
def _execute(
    ...
    with os.fdopen(descriptor, "a+") as lock:
        ...
        if preparation.rate_gate(config) != verified["source"]["rate_gate"]:
            raise ValueError("Pinned index_daily 500 rpm gate changed")
        ...
        if not execute:
            for target in (
                ...
                guards.enter_context(
                    patch(
                        target,
                        side_effect=AssertionError("Offline index_daily batch plan"),
                    )
                )
        verified = preparation.verify_manifest(manifest, manifest_sha256)
