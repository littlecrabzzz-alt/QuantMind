#!/usr/bin/env python3
"""Replace open KPL history days with reviewed month ranges; no source calls."""
...
from backend.shared.tushare_market_sentiment_contracts import (  # noqa: E402
    KPL_TAGS,
...
def _planned_ranges(config, today):
    selected = {
        **config,
        "market_sentiment_apis": [API],
        "planning_epoch": today.strftime("%Y%m%d"),
    }
    for job in iter_market_sentiment_jobs(selected, today):
...
def migrate(pipeline, config, today, *, apply=False):
    ...
    try:
        ...
        for table in ("kpl_daily_open", "kpl_range_coverage", "kpl_daily_covered"):
            db.execute("DROP TABLE IF EXISTS temp." + table)
        db.execute(
            "CREATE TEMP TABLE kpl_daily_open ("
            "job_id TEXT PRIMARY KEY,trade_date TEXT NOT NULL,tag TEXT NOT NULL) "
            "WITHOUT ROWID"
        )
        rows = db.execute(
        ...
        for row in rows:
            ...
            if set(params) != {"trade_date", "tag"}:
                raise ValueError("Unexpected KPL history-day request shape")
            trade_date, tag = params["trade_date"], params["tag"]
            if (
                not isinstance(trade_date, str)
                or not re.fullmatch(r"[0-9]{8}", trade_date)
                or datetime.strptime(trade_date, "%Y%m%d").strftime("%Y%m%d")
                != trade_date
                or tag not in KPL_TAGS
            ):
                raise ValueError("Invalid KPL history-day request")
            daily.append((row["id"], trade_date, tag))
        db.executemany("INSERT INTO kpl_daily_open VALUES(?,?,?)", daily)
        split_daily = _count(
            db,
        ...
        if split_daily:
            raise ValueError("Split KPL history days require separate review")
        ...
        if len(planned_ids) != len(set(planned_ids)):
            raise ValueError("KPL range planner produced duplicate requests")
        db.execute(
            "CREATE TEMP TABLE kpl_range_coverage ("
            "job_id TEXT PRIMARY KEY,tag TEXT NOT NULL,start_date TEXT NOT NULL,"
        ...
        for job_id in planned_ids:
            ...
            if row is None or row["state"] not in RANGE_STATES:
                raise ValueError("Planned KPL range is missing or unusable")
            params = json.loads(row["job"])["params"]
            if set(params) != {"start_date", "end_date", "tag"}:
                raise ValueError("Unexpected KPL range request shape")
            db.execute(
                "INSERT INTO kpl_range_coverage VALUES(?,?,?,?)",
                (job_id, params["tag"], params["start_date"], params["end_date"]),
            )

        db.execute(
            "CREATE TEMP TABLE kpl_daily_covered ("
            "job_id TEXT PRIMARY KEY,range_id TEXT NOT NULL) WITHOUT ROWID"
        )
        db.execute(
            "INSERT OR IGNORE INTO kpl_daily_covered "
            "SELECT day.job_id,span.job_id FROM kpl_daily_open AS day "
            "JOIN kpl_range_coverage AS span ON span.tag=day.tag "
        ...
        old_open = len(daily)
        covered = _count(db, "SELECT count(*) FROM kpl_daily_covered")
        if covered != old_open:
            raise ValueError(
                f"{old_open - covered} KPL history days lack matching range coverage"
            )
        external = _count(
            db,
            "SELECT count(*) FROM partition_children AS edge "
            "JOIN jobs AS parent ON parent.id=edge.parent_id "
            "WHERE edge.child_id IN (SELECT job_id FROM kpl_daily_covered) "
            "AND parent.state='split_pending'",
        )
        if external:
            raise ValueError("Covered KPL days are shared by active split parents")
        candidate_attempts = _count(
            db,
            "SELECT count(*) FROM attempts WHERE job_id IN "
            "(SELECT job_id FROM kpl_daily_covered)",
        )
        superseded = db.execute(
            "UPDATE jobs SET state='superseded' WHERE id IN "
            "(SELECT job_id FROM kpl_daily_covered) AND state='pending'"
        ).rowcount
        if superseded != old_open:
            raise ValueError("Not every covered KPL history day was retired")
...
def _write_receipt(root, report):
    ...
    path = Path(root) / ("kpl-daily-retirement-v1." + _sha(body) + ".json")
    if path.exists() or path.is_symlink():
        if _regular(path, "Existing receipt").read_bytes() != body:
            raise ValueError("Receipt path collision")
        return path
    descriptor, temporary = tempfile.mkstemp(prefix=".kpl-daily-", dir=root)
    try:
