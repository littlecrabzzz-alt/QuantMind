#!/usr/bin/env python3
"""Plan or execute one hash-pinned paid policy/report batch."""
...
from scripts import prepare_tushare_paid_policy_batch as preparation  # noqa: E402
...
def _execute(
    ...
    with os.fdopen(descriptor, "a+") as lock:
        ...
        if preparation.rate_contracts(json.loads(config_bytes)) != verified[
            ...
            raise ValueError("Paid API rate contracts changed")
        ...
        with ExitStack() as guards:
            for target in (
                ...
                guards.enter_context(
                    patch(
                        target,
                        side_effect=AssertionError("Offline paid-policy batch plan"),
                    )
                )
            verified = preparation.verify_manifest(manifest, manifest_sha256)
