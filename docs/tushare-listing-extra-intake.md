# 历史列表、IPO、代码映射与市场统计接入

以下为纯合同设计记录；2026-09-09已完成生产接入，验收见末节。设计时核对共享台账的 263 项固定目录和 13 项额外发现（包含导航/旧页，不能等同 13 个新 API），并与 140 个已注册接口对比。仅选以下 4 个未接入接口，不重复 VIP 别名、互联互通旧页或龙虎榜接口。代码只提供纯合同和离线规划，账户权限全部 `unprobed`。
...
近期先处理最近 7 个自然日，再轮转历史任务。`bak_basic/daily_info` 按精确交易日查询，不先按当前股票/开市日过滤；`new_share` 按发行日期的月内范围，`bse_mapping` 只做无筛选快照。`new_share` 另做一个仅设置当天 `start_date` 的开放上界请求，接收供应商当前已提供的未来发行信息，不捏造未来截止日期，也不再每天重复拉取封顶的全历史快照。日期 anchor 和有限扫描复用现有外层机制，纯 planner 不持久化游标。
...
- `new_share` 日期范围只按发行日拆分；单日到 2000 行时没有合法 `ts_code`、offset 或 limit 参数。仅含 `start_date` 的开放未来请求也没有合法上界可直接二分，两者终端饱和都应保持 blocked。供应商明确终止只证明请求时已知记录，不证明未来发行或旧日期回填不会再出现；发行很久后补充上市日期等修订，也不保证被 7 日范围发现。
...
纯合同后已接入现有 registry、Pipeline、store 与 Mac 镜像安装清单，组名 `listing_extra`，默认禁用。历史证券发现合并存量历史/T股票、IPO及BSE旧/新代码；市场统计类别独立保存。IPO默认按发行日查询，上市日可显式选择；BSE查询仅在原映射列上接受源后缀代码，普通证券查询仍使用内部前缀。专属测试覆盖发现、规划、离线查询及饱和末端不冒充完整。以下记录已完成真实权限、返回字段及云端/Mac端到端验收；默认关闭仅指未配置的新环境。

## 2026-09-09 生产验收

f2cd52e经双端校验部署，4请求1.717秒：bak_basic5566、new_share22、bse_mapping248、daily_info12，共5848行、54文档输出列全部返回。原文/观察/Parquet/固定API及Mac禁网逐字段一致。new_share按ipo_date范围匹配，BSE旧/新原.BJ均保留，映射生效日仍未知。

已启用listing_extra组，近期17项、历史483项首次新任务。自动无筛选IPO发现实际达到2000行，保持blocked/possibly_truncated；独立日期分区继续，不能把此快照当全集。日统计实样本发现文档31类别外的SH_FUND_REITs，发现集合为32，未漏掉新标签；深圳tr列存在允许null。目录215把类别值当input字段的问题已精确修正，所有31类别及起日另存metadata，原HTML哈希不变。

固定版本data-1a593ee58c333ebb02b025b1529f342c914a82cf389ac7878eeb34c73f45d668；证据validation/eight-acceptance.json、eight-api-acceptance.json及/tmp/tushare-eight-mac-verified.json。历史范围、修订、PIT及代码变更时点仍未证明完整。
