...
def main() -> None:
    ...
    for record in records:
        suffix = record["job"]["params"]["ts_code"].rsplit(".", 1)[-1]
        suffixes[suffix] = suffixes.get(suffix, 0) + 1
    if any(suffixes.get(value, 0) for value in ("CFX", "SI", "SW")):
        raise ValueError("Excluded index suffix entered the batch")
    overlap = {
        "schema_version": 1,
        "status": "passed",
        ...
        "excluded_suffix_counts": {key: suffixes.get(key, 0) for key in ("CFX", "SI", "SW")},
    }
