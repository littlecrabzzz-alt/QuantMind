...
| 行情、财务及新增基础数据 | Mac 本地采集到独立源目录；核验来源、schema、清单与内容哈希后向云端发布固定文件版本，两端各自派生，不传业务数据库 |
...
2026-09-27 起以 [本地数据源运行说明](local-market-source.md) 为准：Mac 每天03:00采集，云端仅接收固定版本；不再依赖云端采集和每15分钟反向下载。

复用现有 QuantDB 采集、快照哈希/增量传输、`quantdb_refresh.apply` 和 PG/Qlib/市场分析构建入口。新编排只负责本地隔离容器、阶段重试和已校验版本交付，不传数据库、队列或研究结果。云端保留100GiB、Mac保留300GiB恢复余量。

沙盒空闲时检查相对上次基线的本地修改，有冲突则保留双方、延后应用。APFS暂存、逐文件校验和原子交换完成后才派生行情表、Qlib和市场分析；派生失败保持待恢复状态，下轮继续。固定研究输入不变。新源登记的中转/回滚副本保留两版；迁移前备份不自动清理。

完整业务数据库基线仍由 `.local-dev/SNAPSHOT_ID` 标识；本地行情以 `.local-dev/QUANTDB_SYNC.json` 的 applied 和实际日期验收。云端接收回执在 `/data/local-market-source/A-receipt.json`。采集、发布、送达、派生和研究使用分别报告，下载或安装成功不能替代数据验收。
