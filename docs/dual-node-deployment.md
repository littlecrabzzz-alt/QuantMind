...
部署回归测试 18 项通过，云端快照、Mac 下载和逐文件校验均已完成；尚未安装定时刷新。Mac 已能从固定快照启动隔离全栈，但外部 LLM/行情源仍依赖网络，本地写入也不会自动登记到云端。离线研究产物应单独保留，不能直接回灌数据库；自动结果登记尚未实现。不能把这一状态描述成自动双向数据库同步，或声称离线时仍与云端实时一致。
