...
秒级窗口共享午夜边界，避免依赖未确认的左右包含语义；返回的重复行交给存储层去重。沪深问答近期同时生成问题日期和 `pub_start/pub_end` 回复时间窗口，用于发现旧问题的新回复。动态细分器须识别这两个轴；不能只保留回复轴，否则未回答的问题会丢失。终端时间窗口达到 3000 行上限时，按官方输入参数 `ts_code` 和本地保留的股票身份继续拆分；父响应直接复用，不再请求。股票身份全集仍无供应商独立总数，因此拆分增加实际覆盖，但不宣称历史全集已经证明。
