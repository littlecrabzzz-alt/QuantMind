...
第二个生产批次在固定版 `data-1ed56821ebf8f42ca999dc754829b80fa0ee27cbae3a0501ec399001ee62e10f` 发布后执行。停服后的正式清单与离线审计一致：360项均为 `pending/tries=0`，包含290个已有拆分叶和70个历史根、共215只基金，与首批任务集合无重叠。48.565秒内完成360次请求，结果为133个 `done`、93个 `empty`、134个 `split_pending`；后者继续进入标准日期二分闭包。清单 SHA256 为 `2e82be986b08365bef13a698824c17d977eccdda3e16f32aa75149e5dcd19eed`，收据 SHA256 为 `e4277d3b3537dd65a31982ba03db072bf0bae5a9488bbfd431f4cb48c8b70b35`。执行器没有发布或切换 `CURRENT`，结果等待后续固定版周期统一发布。

第三个生产批次在常规任务自然排空后重新冻结，不复用前两批清单。新清单包含270个当前拆分叶和90个历史根、共225只基金；离线plan-only验证360项后，49.602秒完成360次请求，结果为202个 `done`、39个 `empty`、119个 `split_pending`。清单 SHA256 为 `add2f3cf9c145628fea2e861d908d8940cc1914a32f51244645ec187bb7d91c7`，任务集合 SHA256 为 `84cfbeb5687c11af812d26db9e69b7173c6eb320509003bb4128e77132a3d039`，收据 SHA256 为 `6826ccd3616cdbaa4c900770dd22fd8e7f48629db36ea2dc5861e2655426ebf9`。本批和同窗口财务第八批均在共享锁下执行，配置及 `CURRENT` 前后不变，等待下一固定版周期统一发布。

第四个生产批次在下一固定版及Mac镜像追平后重新冻结，包含238个当前拆分叶和122个历史根、共237只基金，360项全部为 `pending/attempts=0`。49.170秒完成360次请求，得到179个 `done`、41个 `empty`、140个 `split_pending`；清单 SHA256 为 `72549cc8e06d78c03354a2e73d04b24b4dba68455add5e3bc9bf0152436b231a`，任务集合 SHA256 为 `3de26af98ca7f88dddad0df056fb4b050d48645abef2cf5699c1c5b24cf02ad4`，收据 SHA256 为 `94a9a3ce6200fdb9e662ca53bbfecd2eb5ea01c9d027859c10540b9c7521be78`。四批累计1,440次调用，679 `done`、226 `empty`、535 `split_pending`。

对执行前固定版的严格断网读取显示：可查询最新自然键488,037行、529只基金，净值日期为2012-10-23至2026-09-08；相对于22,216只 `fund_basic` 代码，当前有行覆盖率仅2.381%。其中176,964行来自仍开放的截断父分区，只能作为已保存观察值查询，不能计为闭包。固定版仍有374个开放拆分父节点，其中77个被 `empty_unverified` 子节点阻断；普通pending批次无法解决这些阻断，需要独立的空响应复核和闭包政策。按净值日期读取必须显式使用 `date_field='nav_date'`。

第五个生产批次未等待固定版发布，直接在下一独占窗口从停服后的权威队列重新冻结；执行器仍以未变化的 `data-d5d918…` 为起始指针。清单含282个拆分叶和78个历史根、共209只基金，360项全部为 `pending/tries=0`。46.609秒完成360次请求，得到212 `done`、43 `empty`、105 `split_pending`。清单 SHA256 为 `c009b54e9b77eb1fe2d9238a430461c296dacfa6a35667c11a8629250494a052`，任务集合 SHA256 为 `1909131e04bd0c7195bee0d01a295a629dccd5ef351fe40204199a4c9ab3b237`，收据 SHA256 为 `2cdd1fba67d8cbeff8ce9f65433e0d5edec46e4a84e53367f5bddf603d73a287`。五批累计1,800次调用，891 `done`、269 `empty`、640 `split_pending`；新增结果继续等待正常发布。
