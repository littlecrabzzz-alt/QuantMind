...
- `scripts/test_research_agent_gateway.py`：3项通过，未认证或伪造内部身份不能通过公开 Agent 网关；已认证身份沿用既有转发；非数字业务账户沿用平台用户映射登记策略。
...
2026-09-12 发布完成：功能提交 `2f4a541e`、业务账户映射修复 `c2af0fbc`，集成至主分支 `1f54c1d4` 并推送。两端源码/Git handoff 通过；本地 `quantmind-dev-research-agent`、云端 `quantmind-research-agent` 已启动，对应 `quantmind` 网关加载新代码且健康。原采集 worker/Beat 未因本次发布重启。

本地服务节点 `adb51708ac1281182ebe`（本地沙盒研究），云端 `cccd02f077f97fb7a241`（云端研究）。两端已加载的公开网关对无登录、伪造内部身份均返回401；受信任内部转发读取能力返回200/ready=true。云端真实 GLM-5.3-Flash SDK 调用回复“连接正常”（33输入/11输出tokens），未创建业务研究课题，见 `cloud-model-smoke.json`。

本地 Vite 3000 自动加载新源码；云端通过 `scripts/dual-node.sh web-publish` 发布页面，HTTP读取 index 与本地构建逐字节匹配，SHA256 `558158ee10997581ee717950ebcd0748b75b7d8d90ef41b029589d2e9bb8c67c`。Mac 当时锁屏，没有操作用户已有登录会话；交互/成果验收是上文独立浏览器完成的，不能描述为已用用户会话重复执行。

正式发布前已分别保存两端 research_drafts 备份；新原生检查点表由服务幂等创建。原研究、RRG记录、固定输入和其他任务的工作文件保留。临时构建代理只用于从PyPI获取冻结依赖，运行服务不依赖该代理；主服务依赖未升级。
...
- 隔离测试账户为 `1`，现有回测列表组件自动补零成 `00000001`，使列表请求被已有身份校验拒绝；结果详情按真实ID可读取。没有放宽身份校验来掩盖这个问题，已查当前本地业务记录的账户为 `admin`，不触发数字补零；没有把测试夹具的失败写成用户会话已经通过。另已验证 `admin` 映射到平台策略账户 1，修复 Agent 新接口不能直接 int(user) 的问题。
