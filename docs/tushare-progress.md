...
## 2026-09-19 本地分片复核队列提速

`reconciliation_parents_per_tick`只控制本地分片闭合扫描，现已从采集规划指纹排除，避免运行预算变化触发无意义的全量重规划。Mac私有配置保持0600，生产预算由每轮64提高到1000；供应商限速、每日配额、抓取并发和Token均未改变。

首个正式周期在114.044秒内继续完成749次真实Tushare请求和1720个文档阶段，同时用2.088472秒复核1000个分片父任务、确认200个已具备完整子证据的父任务可闭合。规划状态仍为`interval_not_due`，指纹前后相同，周期无错误。`split_pending`仍会因新响应继续拆分而增长，单轮闭合数不代表历史完整；全量补采继续。机器证据为`docs/tushare-partition-reconciliation-production-20260919.json`。
