...
目标：按 [总计划](tushare-integration-plan.md) 和 [任务书](tushare-integration-task.md)，接入账号支持获取的数据、完整保存历史/字段/原文/版本。Mac 是完整归档和唯一全量写入节点，后续可迁移 NAS；云端只保存研究所需子集和缓存。单接口缺权限或文档/历史不完整时记录后继续其他接口。这个文件是当前状态索引，阶段证据追加到共享主树 `coordination/tushare-data/`，不能只靠聊天或内存续接。

## 已验证基线

- 当前生产基线：Mac `~/Library/Application Support/QuantMind/tushare` 保存正式请求、重试、原始响应、观察、Parquet、附件与版本；LaunchAgent 是唯一全量写入者。全历史、修订、PIT 和研究准入仍未完成。
- 云端正式全量源已冻结并标记 `source_paused=true`，仅保留 `tushare-research-cache.timer`；没有全量 archive worker。`pipeline-status.json`、`archive-worker-status.json`、`CURRENT.json` 及 `releases/<id>/manifest.json` 是本地实际运行证据。仓库中的数量只是核查时点，不替代实时状态。

## 当前工作与完整目标

| 工作项 | 状态 | 验收/下一步 |
|---|---|---|
| 全目录逐项台账 | 270 项公开基线、25 项正文发现页；258 个命名范围中 254 个已注册且可本地读取 | 4 个非运行接口分别是2个写操作、1个SDK封装和1个公开只读合同阻塞；41个无采集名页面继续保留原分类证据 |
| 现有回填提速 | tiered_v1 分层限速和500/min账号门已部署；恢复后首轮771次请求/112.639秒 | 具体接口、独立权限及每日配额分别保护；实际吞吐含计划、归档、规范化和文档工作，不能用频次上限直接推算 ETA |
| 已购九个文本 API | 九个合同已部署并在Mac持续回填，原始响应、附件和解析状态分别保存 | 按来源完成历史覆盖、附件失败恢复、空响应和修订验收 |
| 结构化接口 | 254 个命名采集接口已注册并有读取键；生产启用范围、权限和历史闭合仍逐项记录 | 继续处理满页分片、发现宇宙、空响应、修订和未知最早历史，不以注册数冒充数据完整 |
| 非采集命名接口 | 4 个保持显式边界 | `p_save`/`p_delete`不执行，`pro_bar`仅SDK，`ggt_monthly`公开只读合同尚无可用正式HTTP采集路径 |
| PDF/附件、HTML正文与解析 | 本地12路有界下载/4路解析持续运行；恢复后首轮完成1760个阶段 | 安全下载、独立状态/重试和不可变原文；终态失败、加密、无文本及解析积压分别保留 |
| 通用查询与 API/Agent 消费 | 254 个采集接口及6个财务读取别名可本地读取 | 已完成存量数据不依赖Tushare Pro；各接口全历史、单位口径和PIT仍分别验收 |
| 全历史回填、修订与缺口补齐 | RRG、文本、结构化、market四组持续回填；未证实上游全历史完整 | 分接口/来源/标的/日期/字段统计，满页细分、不可证实窗口保留 partial |
| 全范围归档与版本增长 | Mac保存完整归档、旧元数据和不可变release；云端仅研究子集 | NAS迁移前保持本地完整性、版本闭包和故障恢复；不把全量数据库放在云端数据盘 |
| 容量与运行监测 | Mac强制保留300 GiB；当前约1.91 TiB可用，尚缺长期最终规模实测 | 采集、附件、清单和快照按真实增长监测，达到迁移阈值后转NAS，不能用容量缩小采集目标 |
| RRG/训练可用性 | blocked_data | 与可下载/可查询状态独立；历史分类、PIT、交易与持仓暴露验收前不启用 |

## 非阻塞缺口

- 270项基线和25项发现页中仍有41个无采集名页面；分类、SDK、数据交付页和失效正文不能自动发明API。权限不能从积分一概推定。
- 七类基线：在市 ETF 上市日缺失；CI005021 历史成员 N 空响应；行业历史名单/known_at 未证明。
- 官方未公开完整历史下界/分页上限的接口不能宣称取全；明确记录待查，不替换为未经支持的 offset 参数。
- 港股/美股财务独立包已真实拒权；历史分钟仍受独立权限/频次边界。已购文本来源继续逐项验收，不代用户购买新权限。
...
### 2026-09-11 fund price exact batch 13

- 基于新固定版`79a2…`固定2011-04-19至2012-01-09的180个共同SSE开市日，`fund_daily`和`fund_adj`各180项。固定版、日历、配置、preparer、helper和360个任务ID全部哈希冻结，plan-only明确为0 authority、0凭据、0上游、0写入和0发布。55.993秒完成360次HTTP 200，全部done并保留60470行。
- 360个object、360个observation和360个Parquet共1080个唯一引用、9806929字节，逐文件SHA256通过；不确定调用0，未发布或切换`CURRENT`。Worker和Beat恢复healthy。云盘可用223360933888字节，距100 GiB硬线还有115986751488字节。1080项等待后续正常固定版和Mac镜像闭环；完整生命周期、修订、可交易状态、`known_at`和PIT仍未闭合。机器证据为`docs/tushare-fund-price-batch13-20260911.evidence.json`。

### 2026-09-11 financial exact batch 18
...
### 2026-09-11 post-79a2 three-batch fixed-release closure

- 正常publisher任务`9f913ccc-b455-4491-a4c1-aad7b3fc250b`以Celery SUCCESS、上游请求0完成，174.804秒原子发布`data-8f6c6d8256db0794b65e737aac9a490075d0d17152b05ab76a9af59e369e95f2`；327170247字节manifest含111201个dataset、823920个文件，SHA256与CURRENT一致并包含retained observations。
- Mac LaunchAgent第191轮下载9003个增量文件，完整验证823920项、stderr 0、exit 0后原子切换。固定版验收器对基金行情13、财务18和基金份额11的3102个唯一引用、31204974字节全部通过：manifest缺失0、元数据错误0、本地物理错误0。
- 生产镜像在`--network none`、socket/DNS阻断、空Token和只读镜像条件下读取`index_daily`、`fund_daily`、`fund_adj`、`fund_share`及三张财务表各3行，上游调用0。云盘可用222423474176字节，距100 GiB硬线还有115049291776字节；API、Worker和Beat healthy。三批已从“权威端闭环”提升为“本地固定版离线可用”。完整历史、修订、`known_at`、PIT成员、RRG可交易状态和PCF仍未闭合。机器证据为`docs/tushare-fixed-release-8f6-three-batches-20260911.evidence.json`。

### 2026-09-11 fund price exact batch 14 partial closure
...
## 2026-09-12 05:32 index_daily 11-13 / index_weight 7-9 exact window

- Beat was stopped first and the in-flight ordinary task drained naturally. Six exact batches then ran serially against fixed release `data-76b031903c08e660d869cb8ef8218dab6cca8bd23cc8811adcf55adbdd8d0898`; no exact batch published or switched `CURRENT.json`.
...
## 2026-09-12 07:29 index_daily 17 / index_weight 13 exact window

- After the standard no-revoke drain, `index_daily` batch 17 was repinned to ea64 because seven tasks had changed since the pre-publication candidate. The old candidate was retained unchanged; the new 360-item manifest passed live-pristine and zero-overlap gates. It completed 360 HTTP-200 calls in 66.574 seconds as 358 done and two empty, retaining 81,308 rows.
...
2026-09-12 10:12 CST：3861固定版后排空普通采集，重新冻结`index_daily`第19批360个pristine任务；与17份历史manifest的5820任务在task/logical/request三层重叠均0。45.347秒完成360次HTTP 200，355 done/5 empty、85171行，不发布、不切CURRENT；独立闭包逐SHA验证1075引用/19916437字节、物理错误0，6份不可覆盖证据位于`validation/index-daily-batch-20260912/batch-19-*`。一次远端shell变量提前展开让新证据暂落文件系统根目录，未覆盖既有或权威数据，已立即移入目标目录并确认根目录无残留。worker/Beat恢复healthy、restart0、OOM false；本批等待下次正常固定发布与Mac镜像。证据：`docs/tushare-index-daily19-20260912.evidence.json`。

2026-09-12 10:56 CST：专用`index_weight`后代合同已由`01a38228`进入master；生产窗口重新验证并冻结180个完整兄弟对/360个pristine叶任务。47.044秒完成360次HTTP 200：180 empty、180 split pending、289938行，生成360个pristine直属孙任务；图从540任务/360边增至900任务/720边。独立闭包逐SHA验证900个物理引用/26636330字节、错误0；不发布、不切CURRENT，worker/Beat按原容器恢复healthy、restart0、OOM false，普通采集已恢复。约557秒只读预检是后续性能优化项，先采集分阶段耗时再改代码；当前批与`index_daily`第19批一起等待正常固定发布、Mac镜像和精确验收。空结果、继续拆分的孙任务、完整历史、修订、`known_at`、PIT语义及RRG分类仍未闭合。证据：`docs/tushare-index-weight-descendant-batch1-20260912.evidence.json`。

2026-09-12 11:19 CST：正常任务`5ff83922…`以`publish_only`、0上游、268.975秒发布`data-77bb1b4c…`；397554805字节manifest含906518文件/126008数据集。云端对`index_daily`第19批与`index_weight`后代第1批合计1975引用/46552767字节逐SHA通过。Mac第250轮因增长后的manifest超过旧180秒绝对传输上限而安全保留3861；`8fee86a3`仅将该绝对上限调至900秒，保留rsync 45秒无活动超时、临时暂存和原子切换，13项测试通过且安装副本哈希一致。真实第251轮越过旧上限，下载4127个增量文件、完整验证906518项，退出0、stderr空并于11:16:46切到77bb；本地同一1975引用全部通过，空Token与socket/DNS阻断下可读`index_daily` 3×17和`index_weight` 3×11，上游0。worker/Beat健康、restart0、OOM false；360个新孙任务、空结果、完整历史、修订、`known_at`、PIT语义和RRG分类继续开放。证据：`docs/tushare-fixed-release-77bb-index-round-20260912.evidence.json`。

2026-09-12 11:40 CST：`index_weight`后代第2批在优化后的只读门上冻结180个完整兄弟对/360个pristine叶任务；将360次无前导索引的logical-key扫描合并为一次跨epoch批量查询后，生产inspect由约557秒降至15.902秒，直接查询基准2.982秒，降幅97.1%，`e540b05e`与`53ba7a9e`的23项定向测试通过。精确窗口45.865秒完成360次HTTP 200：180 empty、180 split pending、285105行，生成360个pristine直属孙任务；图从540任务/360边增至900任务/720边。独立只读闭包逐项核对job/attempt/result并验证900个唯一引用、26188287字节全部SHA通过，六份证据以不可覆盖方式归档。首次闭包因一次性脚本路径正则多转义而在产物和authority写入前退出，修正后通过。worker/Beat恢复healthy、restart0、OOM false；本批等待正常固定发布、Mac镜像和本地精确验收，深层后代、空结果确认、完整历史、修订、`known_at`、PIT语义及RRG分类仍开放。证据：`docs/tushare-index-weight-descendant-batch2-20260912.evidence.json`。

2026-09-12 11:52 CST：Beat先停，两条已接收普通任务分别自然完成后worker退出，全程无revoke/kill；优化后的只读门用3.76秒冻结并检查`index_weight`后代第3批180个完整兄弟对/360个pristine叶任务。生产精确窗口45.982秒完成360次HTTP 200：180 empty、180 split pending、287004行，生成360个pristine直属孙任务；图从540任务/360边增至900任务/720边。独立闭包逐项核对job/attempt/result并验证900个唯一引用、26363366字节全部SHA通过，六份材料不可覆盖归档。首次execute只因root临时目录的shell输出重定向权限在runner、凭据、上游和authority写入前拒绝，随后同一冻结pin在生产容器成功。worker/Beat恢复healthy、restart0、OOM false；第2/3批等待正常固定发布、Mac镜像和本地精确验收，深层后代、空结果确认、完整历史、修订、`known_at`、PIT语义及RRG分类仍开放。证据：`docs/tushare-index-weight-descendant-batch3-20260912.evidence.json`。

2026-09-12 12:17 CST：正常任务`acb364dc…`以`publish_only`、0上游、273.557秒原子发布`data-385ae0c5…`；401323613字节manifest含910451文件/126657数据集并保留observations。`index_weight`后代第2/3批合计1800个唯一引用（720 object、720 observation、360 Parquet）/52551653字节在云端逐文件通过，缺失、元数据、物理SHA错误均0，证据不可覆盖归档。Mac LaunchAgent第254轮下载3932个增量文件、完整验证910451项，exit0、stderr空并于12:16:42切换；第二次本地精确复核同样全部通过。空Token且阻断socket连接、create_connection与DNS后可读`index_weight` 3行4列，上游0，确认两批存量数据不依赖Tushare Pro。worker/Beat保持healthy、restart0、OOM false；独立恢复快照继续后台传输且本地`latest`尚未切换。完整历史、720个新孙任务、空结果确认、修订、`known_at`、PIT语义及RRG分类仍开放。证据：`docs/tushare-fixed-release-385a-index-weight-descendants2-3-20260912.evidence.json`。

2026-09-12 12:29 CST：Beat先停，已运行的普通任务`14040b88…`用252.692秒自然完成后worker退出，全程无revoke/kill；`fund_share`第16批按385a固定版冻结沪深各180、共360个pristine日期请求，覆盖2019-04-06至2019-10-03，与14份历史manifest的4920个请求在task/logical/request三层重叠均0。44.282秒完成360次HTTP 200：251 done、109 empty、59951行、不确定调用0，观测487.783 rpm。独立闭包逐SHA验证971个唯一引用/6092611字节，六份材料不可覆盖归档；操作捕获中队列预检产生的单行前缀在解析前被单独剥离，原始receipt未改。worker/Beat精确恢复healthy、restart0、OOM false；本批等待正常固定发布与Mac镜像，空结果确认、完整历史、修订、`known_at`及PIT覆盖仍开放。证据：`docs/tushare-fund-share-batch16-20260912.evidence.json`。

2026-09-12 12:46 CST：发现独立文档worker自9月10日一次正常warm shutdown后漏恢复；精确启动原容器后Celery按110秒expiry丢弃536条过期调度消息，首个新任务随后因90秒工作后对1.8GiB/约122万行文档库做约29秒全表状态汇总而触发110秒硬限，容器未OOM或重启。`25d81508`新增schema v3覆盖索引；65项文档测试、Ruff、编译及diff检查通过。Beat先停、普通任务`97bfe69f…`用255.892秒自然结束后完成25.258秒生产迁移，汇总降至0.119秒且查询计划命中covering index。恢复三服务后真实任务`f1e48302…`以Celery SUCCESS在90.613秒推进77个阶段（40下载、37解析），pending由1218717降至1218679、parsed由20456升至20490；扩容后可用337107091456字节、磁盘状态healthy。普通worker、文档worker及Beat均healthy、restart0、OOM false；约121.9万正文仍是长期积压，不声称语料完整。证据：`docs/tushare-document-worker-recovery-20260912.evidence.json`。
...
## 2026-09-12 每日配额保护上线与全范围补证

master560ccefd/13dce280/d61e93d6已push、双端源码与Git对齐。复用原API/day账本将cyq_chips纳入每日保护，cyq_perf原激活日2026-09-10及9月11日176/9月12日104次计数不变。chips于9月12日激活，因当日旧调用量未知，门控到北京时间9月13日00:00；真实Pipeline精确scope验收0HTTP、0凭据读取，原任务pending/tries0/resultNULL和attempts空完整保留。状态used=null且不报告剩余额度。坏计数或非法日期失败时清除任何旧remaining，不伪装为可用。当前两接口每日200000；12月5日预计8100积分后20000，84天到期提醒仍可观察。
...
## 2026-09-12 960e固定版闭合index_daily第21批

正常publisher任务58f73731以publish_only、0上游调用、337.704秒发布data-960e25bc；manifest447388279字节，包含976965文件、135299数据集并保留observations。第21批1006个指定引用/15520675字节在云端缺失0、metadata错误0、物理SHA错误0。
...
## 2026-09-12 top10股东第3批权威端闭包

并行只读候选的唯一非阻塞共享锁尝试遇普通worker写锁，按合同退出且没有候选、凭据、上游或写入。随后受控窗口自然排空普通worker，正式preparer固定960e并以batch1/2为历史，冻结180个完整兄弟对/360个pristine history任务；两接口各180、SH/SZ/BJ各120，报告期均为20071231。plan-only五个would字段全false，与前两批720任务在task、logical、canonical request和pair四层重叠均0。
...
## 2026-09-19 港股/美股财务独立权限实测

用港股腾讯和美股苹果、四类财务报表接口共8次有界真实请求验证当前账号。`hk_income`、`hk_balancesheet`、`hk_cashflow`、`hk_fina_indicator`、`us_income`、`us_balancesheet`、`us_cashflow`、`us_fina_indicator` 全部返回拒权，因此保持 `enable_foreign_financial=false`，没有生成约27.4万个必然失败的全历史任务。全部原始响应和观察证据本地保存，Token未进入报告或Git；机器证据为 `docs/tushare-foreign-financial-permission-production-20260919.json`。

这只证明当前账号没有港股/美股财务独立包，不影响已经启用的港股行情、新闻、公告等不同权限接口。后续权益变化时必须重新有界验证，不能用本次结果永久缩小目录目标。

## 2026-09-19 WZ/GZ显式历史范围去重

生产配置已明确从`19900101`采集。`wz_index`和`gz_index`的39个终态范围任务分别连续覆盖到`20260919`，因此显式范围下不再额外生成空参数全历史请求；没有配置历史起点时仍保留空参数发现。旧`wz_index`空参数响应在3000行时明确`has_more=true`，属于截断请求而非完整历史。

零上游迁移逐SHA验证180个对象、观察和Parquet文件后，只把这一条旧请求从`blocked`改为`superseded`，原结果和一次attempt保持不变，1990年前是否存在历史仍明确未知。完整套件1394项通过；部署后首轮完成771次真实请求和1760个文档阶段，`blocked`从781降到780。Mac、origin和云端Git已对齐，云端研究缓存timer继续运行且全量写入进程为0。机器证据为 `docs/tushare-wz-scoped-planning-production-20260919.json`。

## 2026-09-19 本地分片复核扫描优化

`reconciliation_parents_per_tick`只控制本地分片闭合扫描，现已从采集规划指纹排除，避免运行预算变化触发无意义的全量重规划。Mac私有配置保持0600，总预算由每轮64提高到1000，其中936优先扫描仍为`split_pending`的父任务，64用于复核已闭合文件；blocked和superseded不再被周期性重复扫描。供应商限速、每日配额、抓取并发和Token均未改变。

第一版旧报告的`resolved=200`没有区分新增闭合与重复确认，不能解释为新增闭合200个。补充计数后，未分流扫描1000项中新增闭合0、重复确认543、耗时9.724111秒；最终分流周期扫描936个open父任务，新增闭合0，另复核64个已闭合父任务且全部保持有效，总复核耗时降到1.679375秒。同轮完成708次真实Tushare请求和1220个文档阶段，总周期107.758秒、无错误、没有重规划。

这说明当前`split_pending`主要仍有真实子分片缺口，不是单纯等待本地回收的虚高。后台扫描继续负责崩溃恢复和已闭合完整性复核，实际子分片由主抓取持续完成；本次不宣称历史完整。机器证据为`docs/tushare-partition-reconciliation-production-20260919.json`。
