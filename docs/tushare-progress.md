...
- 第一批正常联网任务完成360次请求，采集任务86.354秒；后续旧Connect三接口各再完成3个日分区。13:24只读最近1000次attempt均HTTP200、错误0、HTTP429为0。固定版`data-fa83b240278814d12c3762bae8800bdba976df0879b9db6faf272279e619ed92`已包含探测raw/observation与新合同，标准Mac镜像补3123文件后共校验551631文件并原子切换；该固定版早于后续常规历史分区，新的归一化行需等待下一次固定发布和Mac追平后才算离线闭包。
- 当前`master`、GitHub和云端源码统一到`5cd67a1a0197853d1c353981eec61c9e2227119c`；最终Python3.10完整Tushare回归922项通过、skipped5。两个专属worker均healthy，云端Tushare目录约81G、磁盘剩余约152GiB。完整历史、旧月度Connect、实时独立权限、8个新目录接口实测、百万附件、RRG成员PIT/ETF映射与后续固定版离线闭包继续推进。
