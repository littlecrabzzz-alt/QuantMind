...
| 现有回填提速 | tiered_v1 分层限速/灰度500已部署；最新恢复首轮360次/69.914秒（整轮耗时） | 账号500/min，具体接口、独立权限及每日配额分别保护；实际吞吐含计划/发布开销，不能用上限直接推算 ETA |
...
| 通用查询与 API/Agent 消费 | 244 个采集接口已注册；已完成批次有固定版云/Mac读取证据，实际启用与缺口见下方增量及台账 | 新接口的逐项 HTTP/Agent 消费、全历史及单位口径仍分别验收 |
