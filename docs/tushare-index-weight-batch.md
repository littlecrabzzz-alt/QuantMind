...
`prepare_tushare_index_weight_batch.py` 在一个 SQLite 只读事务中只选择现有 `history`、`market`、`pending` 任务。每个指数先取未拆分的完整自然月窗，再按日期由近到远；完整月窗耗尽后才回退到拆分后代或部分窗口。跨指数仍按请求轮次和供应商代码后缀交错，防止 SH 指数长期挤占 CSI 指数。它不创建任务、不读取凭据、不调用上游，旧选择策略生成的manifest仍可验证。
...
对 `data-d5d918ce…` 的断网逐文件审计验证259个Parquet及其路径、大小和SHA，总计11,281,947字节。原始254,697行按自然键得到252,470条最新观察，覆盖2018-02-28至2026-08-31、156个指数、258个指数日期快照和11,399个成分；2,227个重复自然键来自父子区间重叠，该固定版内权重相同。除 `000001.SH` 有103个日期外，其余155个指数都只有1日。

当前latest中222,350行、即88.1%，仍来自 `possibly_truncated` 响应。120个开放拆分节点形成239个叶，其中238个pending、1个empty；日期二分已经实证会把同一交易日的2,227个成分重复到子区间，无法解决单日超过行数上限且接口没有 `con_code` 过滤的问题。因此拆分后代继续保留gap，不以重复值一致宣称闭包。

从固定planner状态离线重建的旧策略下一360项覆盖203个已规划指数，沪市246项、CSI 114项；其中326项是尚未采集的6—8月完整月窗，34项是17个饱和指数的拆分子任务。新版选择已经把同指数的完整月窗放在拆分后代之前，第二批定位为扩大指数和月份覆盖，不能表述为拆分闭包、完整指数宇宙、PIT或研究就绪；完整月窗不足时仍会明确回退到低边际拆分后代，而不是伪造额外覆盖。
