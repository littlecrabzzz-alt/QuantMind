...
python3 scripts/tushare_pipeline.py verify --root "$HOME/Library/Application Support/QuantMind/tushare"
python3 scripts/tushare_pipeline.py query --root "$HOME/Library/Application Support/QuantMind/tushare" --api-name fund_adj
python3 scripts/tushare_mirror.py --install-launchagent
```

Mac 查询需已安装 DuckDB、PyArrow、httpx；采集在现有云端镜像使用项目已有依赖。安装器把不含密钥的客户端文件部署到 `~/Library/Application Support/QuantMind/tushare-client`，后台日志在相邻 `logs/`，源码变更后重跑安装器更新副本。LaunchAgent 只启动镜像脚本，不读 token、不触发 Tushare。Mac 休眠/断网期间不会同步，恢复后下一轮补拉；云端采集独立继续。固定研究输入显式保存 release_id，避免运行中跟随 CURRENT 变化。
