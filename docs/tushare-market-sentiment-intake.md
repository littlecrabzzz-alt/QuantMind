...
运行候选（默认关闭）：`market_sentiment` family 通过 `enable_market_sentiment` 显式选择，配置继续使用 `market_sentiment_apis`、`market_sentiment_history_start`/`history_start`，未设历史下界保持 gap。首次规划不依赖板块发现；一天 46 种合法请求完整保留，近期按日并保留非 A 股日历日期。自然键之外继续保存原始行身份和不可变请求的 `idx_type`、`tag`、`market`、`hot_type`、`is_new`，相同响应在不同请求维度下不会合并。缺少请求身份的固定版读取失败，不能默认成 A 股。

TDX/KP 板块分别投影为 `TDX:<source>`/`KP:<source>`，THS 行业/概念榜分别为 `THS:I:`/`THS:N:`；ETF/基金统一 `FUND:`，转债 `CB:`，期货 `FUT:`。只对请求确属 A 股的合法 SH/SZ/BJ（含历史 T）代码转交易所前缀，对港股合法五位代码（含历史 !）转 HK 前缀，美国标的保留原字符串并加 US 前缀。未知拼写仍带所属资产前缀，未知请求市场为 `UNVERIFIED_MARKET:`。所有 `source_ts_code`/`source_con_code` 保持来源原值，跨数据集关联需同时核对市场和日期语义。

板块发现读取已有原文及饱和尝试：TDX 与 KP 各自独立集合；KPL 股票扇出包含既有历史/退市/T/来源观测股票和合法成分代码。热榜不进入股票发现，也不凭股票全集拆分跨市场热榜。合法范围可二分；单日先按已观测板块或股票扇出，完整标的全集依然未知；单板块成分第二维和热榜过滤全集未实现，饱和保持 blocked/gap。固定版读取默认 `trade_date`，保留 101 列、数字头字段、额外来源列、原始单位和字段级待核验项。新增数据集不改变 release schema，也不重写旧数据。

隔离验证：7 项纯规划测试与 6 项运行测试覆盖原文行身份→normalize→publish→固定版离线读取、全 46 请求变体、数字头列选择、来源代码和跨市场隔离、历史发现、重复观察去重/不同来源行保留、默认关闭、缺列/缺身份、家族校验隔离及饱和不假完成。没有真实权限探测、启用或生产写入；101 列已接线不代表上游实际全部返回。全量覆盖 fixture 的汇总数量由父任务与同期其他候选串行整合，当前分支自身应为 186，联合 6 个跨资产接口后为 192。
